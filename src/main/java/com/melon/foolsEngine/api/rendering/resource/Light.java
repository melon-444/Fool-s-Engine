...
public class Light {
    ...
    public static Light directional(Vector3f color, Vector3f direction) {
        return new Light(DIRECTIONAL, color, new Vector3f(direction).normalize(), new Vector3f(), 1.0f, 0f, null, null);
    }

    public static Light directional(Vector3f color, Vector3f direction, float intensity) {
        return new Light(DIRECTIONAL, color, new Vector3f(direction).normalize(), new Vector3f(), intensity, 0f, null, null);
    }

    public static Light directional(Vector3f color, Vector3f direction, float intensity,
                                    List<RenderTarget> shadowMaps, List<Matrix4f> lightSpaceMatrices) {
        return new Light(DIRECTIONAL, color, new Vector3f(direction).normalize(), new Vector3f(), intensity, 0f, shadowMaps, lightSpaceMatrices);
    }

    public static Light point(Vector3f color, Vector3f position) {
        return new Light(POINT, color, new Vector3f(), new Vector3f(position), 1.0f, 0f, null, null);
    }

    public static Light point(Vector3f color, Vector3f position, float intensity) {
        return new Light(POINT, color, new Vector3f(), new Vector3f(position), intensity, 0f, null, null);
    }

    public static Light point(Vector3f color, Vector3f position, float intensity,
                              List<RenderTarget> shadowMaps, List<Matrix4f> lightSpaceMatrices) {
        return new Light(POINT, color, new Vector3f(), new Vector3f(position), intensity, 0f, shadowMaps, lightSpaceMatrices);
    }

    public static Light spot(Vector3f color, Vector3f direction, Vector3f position, float cutOff) {
        return new Light(SPOT, color, new Vector3f(direction).normalize(), new Vector3f(position), 1.0f, cutOff, null, null);
    }

    public static Light spot(Vector3f color, Vector3f direction, Vector3f position, float cutOff, float intensity) {
        return new Light(SPOT, color, new Vector3f(direction).normalize(), new Vector3f(position), intensity, cutOff, null, null);
    }

    public static Light spot(Vector3f color, Vector3f direction, Vector3f position, float cutOff, float intensity,
                             List<RenderTarget> shadowMaps, List<Matrix4f> lightSpaceMatrices) {
        return new Light(SPOT, color, new Vector3f(direction).normalize(), new Vector3f(position), intensity, cutOff, shadowMaps, lightSpaceMatrices);
    }

    public boolean castsShadow() {
        return !shadowMaps.isEmpty() && !lightSpaceMatrices.isEmpty();
    }
}
