...
public class TestBackend {
    ...
    public static void main(String[] args) {
        ...
        foolsEngine.mainWindow.setTitle("测试");
        ...
        Transform trans = new Transform(position,new Quaternionf(),new Vector3f(0.1f,0.1f,0.1f));
        ...
        foolsEngine.mainWindow.show();
        frame.init();
        frame.setBackGroundColor(0,0.75f,0,1);

        int deg = 0;

        foolsEngine.mainWindow.setFullscreen(false);

        while(!foolsEngine.mainWindow.shouldClose()){

            frame.beginFrame();
            frame.setCamera(camera);
            frame.submit(new RenderCommand(dragon_mesh,material,trans.getMatrix()));
            frame.endFrame();
            foolsEngine.mainWindow.update();
        ...
        shader.destroy();
        manager.destroyWindow(foolsEngine.mainWindow,true);
    }
